<?php

use Difference\Fun\Config;

use Difference\Fun\Module\Parse;
use Difference\Fun\Module\Data;
use Difference\Fun\Module\Dir;
use Difference\Fun\Module\Core;
use Difference\Fun\Module\File;

function function_cache_clear(Parse $parse, Data $data){   
    $object = $parse->object();
    $temp_dir = $object->config('framework.dir.temp');
    $dir = new Dir();
    $read = $dir->read($temp_dir, true);
    $parse = new Parse($object);
    if(
        $object->config('ramdisk.size') &&
        empty($object->config(Config::POSIX_ID))
    ){
        $command = \Difference\Fun\Cli\Cache\Controller\Cache::RAMDISK_CLEAR_COMMAND;
        $execute = $parse->compile($command);
        echo 'Executing: ' . $execute . "...\n";
        Core::execute($object, $execute, $output);
        echo $output . PHP_EOL;
        ob_flush();
    }
    if($read){
        $id = $object->config(Config::POSIX_ID);
        foreach($read as $file){
            if($file->type === Dir::TYPE){
                $file->number = false;
                if(is_numeric($file->name)){
                    $file->number = $file->name + 0;
                }
                if(
                    $file->number !== false &&
                    file_exists($file->url) &&
                    empty($id)
                ){
                    Dir::remove($file->url);
                    echo 'Removed: ' . $file->url . PHP_EOL;
                }
                elseif(
                    $file->number !== false &&
                    file_exists($file->url) &&
                    $id === $file->number
                ){
                    Dir::remove($file->url);
                    echo 'Removed: ' . $file->url . PHP_EOL;
                }
            }
        }
    }
    if(File::exist($object->config('project.dir.vendor') . 'Doctrine')){
        $cacheDriver = new \Doctrine\Common\Cache\ArrayCache();
        $cacheDriver->deleteAll();
    }
    opcache_reset();
    opcache_invalidate('/Application/vendor/difference_fun/framework/src/Module/Parse.php', true);
    File::permission($object, [
        'dir' => $temp_dir,
        'dir_www' => $temp_dir . '33/'
    ]);
}
