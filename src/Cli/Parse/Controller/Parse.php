 ...
 *  -    all
 */
namespace Difference\Fun\Cli\Parse\Controller;
...
use Difference\Fun\Module\Parse as Parser;
        ...
        try {
            ...
            if (File::exist($template_url)) {
                ...
                if ($read) {
                    ...
                    $parse->storage()->data('difference.fun.parse.view.url', $template_url);
                    $parse->storage()->data('difference.fun.parse.view.mtime', $mtime);

                    unset($data->{App::NAMESPACE});
                    $read = $parse->compile($read, $data, $parse->storage());
                    $object->set('script', \Difference\Fun\Module\Parse::readback($object, $parse, App::SCRIPT));
                    $object->set('link', \Difference\Fun\Module\Parse::readback($object, $parse, App::LINK));
                    if($is_json){
