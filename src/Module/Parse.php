...
use Package\Difference\Fun\Raxon\Service\ForkMultiple;
use stdClass;

use Difference\Fun\App;
use Difference\Fun\Config;

use Difference\Fun\Module\Parse\Build;
use Difference\Fun\Module\Parse\Literal;
...
//                        $this->object()->config('parse.compile.disable.function.Parse::prepare_code', false);
        ...
        elseif($type === 'object'){
            ...
            if($this->useThis() === true){
                $source = $storage->data('difference.fun.parse.view.source');
                if(empty($source)){
                    $file = $storage->data('difference.fun.parse.view.url');
                } else {
                    $file = $storage->data('difference.fun.parse.view.source.url');
                }
                if($this->key){
...
//                    $string->result = $string->{'#parallel'};
        ...
        elseif($type === 'string' && stristr($string, '{') === false){
            ...
            if(empty($source)){
                $options = [
                    'source' => $storage->data('difference.fun.parse.view.url')
                ];
                $url = $build->url($string, $options);
            } else {
                $options = [
                    'source' => $storage->data('difference.fun.parse.view.source.url'),
                    'parent' => $storage->data('difference.fun.parse.view.url')
                ];
                $url = $build->url($string, $options);
            }
            $string = Literal::uniform($object, $string);
            $storage->data('difference.fun.parse.compile.url', $url);
            if($this->useThis() === true){
            ...
            $mtime = $storage->data('difference.fun.parse.view.mtime');
...
//                    $string = Parse::comment($string, 'is_disabled: ' . $is_disabled);
    ...
    public static function prepare_code(App $object, $storage, $string): string
    {
        ...
        $uuid = Core::uuid();
        $storage->data('difference.fun.parse.compile.remove_newline', true);
        $string = str_replace(
            [
                '{',
                '}',
