...
class Literal {
    ...
    public static function apply(Data $data, $string=''): ?string
    {
        ...
        $key = $data->data('difference.fun.parse.literal.key');
        if(empty($key)){
            $uuid = sha1('{literal}');
            $data->data('difference.fun.parse.literal.key', $uuid);
        }
        if(isset($explode[1])){
            $temp = explode('{/literal}', $explode[1], 2);
            $uuid = sha1($temp[0]);
            $data->data('difference.fun.parse.literal.' . $uuid, $temp[0]);
            $temp[1] = 'literal-' . $data->data('difference.fun.parse.literal.key') . '-' . $uuid . $temp[1];
            $explode[1] = $temp[1];
    ...
    public static function restore(Data $data, $string=''): mixed
    {
        ...
        elseif(is_array($string)){
            ...
            $tag = 'literal-' . $data->data('difference.fun.parse.literal.key') . '-';
            $explode = explode($tag, $string, 2);
            if(isset($explode[1])){
                $key = substr($explode[1], 0, 40);
                $string =  str_replace($tag . $key, $data->data('difference.fun.parse.literal.' . $key), $string);
                $explode = explode($tag, $string, 2);
                if(isset($explode[1])){
