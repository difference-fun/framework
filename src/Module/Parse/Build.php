...
namespace Difference\Fun\Module\Parse;
...
use Difference\Fun\Exception\PluginNotFoundException;
use Difference\Fun\Exception\PluginNotAllowedException;
use Difference\Fun\Exception\FileWriteException;
use Difference\Fun\Exception\FileAppendException;
use Difference\Fun\Exception\FileMoveException;
    ...
    public function document(Data $data, $tree=[], $document=[]): array
    {
        ...
        foreach($tree as $nr => $record){
            ...
            if(
                ...
                if($remove_newline && $data->data('difference.fun.parse.compile.remove_newline') !== false){
                    $explode = explode("\n", $record['value'], 2);
                    if(count($explode) == 2){
                        $temp = trim($explode[0]);
                        if(empty($temp)){
            ...
            elseif($record['type'] == Token::TYPE_CURLY_CLOSE){
                switch($type){
                    case Token::TYPE_STRING :
                        if($select['value'] == 'if'){
                            throw new Exception('if must be a method, use {if()} on line: ' . $select['row'] . ', column: ' .  $select['column']  . ' in: ' .  $data->data('difference.fun.parse.view.url') );
                        } else {
                            throw new Exception('Possible variable sign or method missing (), "' . $select['value'] . '" on line: ' . $select['row'] . ', column: ' .  $select['column']  . ' in: ' .  $data->data('difference.fun.parse.view.url'));
                        }
...
//                      $run[] = $this->indent() .
    ...
    private function createHeader($document=[]): array
    {
        ...
        $document[] = ' * @author                   Difference\Fun\Module\Parse\Build';
